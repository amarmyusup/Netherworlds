...
module.exports = function quotes(input) {
	return new Promise((resolve, reject) => {
		fetch('https://jagokata.com/kata-bijak/' + input.replace(/\s/g, '_') + '.html?page=1')
			.then(res => res.text())
			.then(res => {
				...
				data.splice(2, 1)
				if (data.length == 0) return resolve({ creator: '@AMY - Amar Maulana Yusup', status: false })
				resolve({ creator: '@AMY - Amar Maulana Yusup', status: true, data })
			}).catch(reject)
	})
}
