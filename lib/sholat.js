...
function sholatAll(kota) {
    return new Promise(async (resolve, reject) => {
        let html = await _axios('https://m.dream.co.id/jadwal-sholat' + kota.replace(/\s/g, '-').toLowerCase())
        $ = cheerio.load(html)
        let data = []
        ...
        }); resolve({ creator: '@AMY', status: true, data })
    })
}
...
module.exports = (kota) => {
    return new Promise(async (resolve, reject) => {
        ...
        if (!sholat.status) return resolve({ creator: '@AMY', status: false })
        let now = moment(new Date * 1).format('DD-MM-yy')
        let data = sholat.data.find(v => v.tanggal == now)
        if (typeof data == 'undefined') return resolve({ creator: '@AMY', status: false })
        resolve({ creator: '@AMY', status: true, data })
    })
}
